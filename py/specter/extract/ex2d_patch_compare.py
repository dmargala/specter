...
def ex2d_patch(image, ivar, psf, specmin, nspec, wavelengths, xyrange=None,
    ...
    #paste in resolution_from_icov for debugging
    #assume no ndecorr for now
    decorr = None
    ###################
    """
    ...
    fluxivar = norm_vector**2  #- Bolton & Schlegel 2010 Eqn 13
    ####################
    ...
    Wx = scipy.sparse.spdiags(wx, 0, len(wx), len(wx)) #scipy sparse object

    #print("Wx")
    #print(Wx)

    ####################################################################################
    #we now return to our regularly scheduled extraction

    #make sure we are using our manually installed cupy fix to get this dot product/transpose right
    #of course we're not even on the gpu yet...
    ...
    #R depends on norm_vector_cpu, Q_cpu, v_cpu, d_cpu, u_cpu, u, v, iCov_cpu, Ax, Wx, W, A, image, ivar... basically everything

    #in specter there is no multipliction by a Q term ?
   ...
   # if full_output:
   #     results = dict(flux=fx_cpu, ivar=varfx_cpu, R=R_cpu, xflux=f_cpu, A=A, iCov=iCov_cpu)
   #     results['options'] = dict(
   #         specmin=specmin, nspec=nspec, wavelengths=wavelengths,
   #         xyrange=xyrange, regularize=regularize, ndecorr=ndecorr
   #         )
   ...
   # else:
   #     return rflux, fluxivar, R
   #     #return fx_cpu, varfx_cpu, R_cpu

    #moment of truth

    R_diff = np.abs(R - R_cpu)
    #print("R_diff")
    #print(R_diff)
    max_R_diff = np.max(R_diff)
    print("max_R_diff")
    print(max_R_diff)
    #~ 1.37537980394105e-07

    #ivar_diff = np.abs(fluxivar - varfx_cpu)
    ##print("ivar_diff")
    ##print(ivar_diff)
    #max_ivar_diff = np.max(ivar_diff)
    #print("max_ivar_diff")
    #print(max_ivar_diff)

    rflux_diff = np.abs(rflux - fx_cpu)
    #print("rflux_diff")
    #print(rflux_diff)
    max_rflux_diff = np.max(rflux_diff)
    print("max_rflux_diff")
    print(max_rflux_diff)

    assert np.allclose(iCov.todense(), iCov_cpu.todense()) #passes
    assert np.allclose(norm_vector, norm_vector_cpu) #passes
    assert np.allclose(sqrt_icov, Q_cpu) #passes
    assert np.allclose(fluxivar, varfx_cpu) #works ok if we use the same definition as specter
    #assert np.allclose(R, R_cpu) #fails, off by a little, probably due to eigen_compose
    assert np.allclose(xflux, f_cpu) #passes
    #assert np.allclose(rflux, fx_cpu) #fails, depends on R so makese sense
